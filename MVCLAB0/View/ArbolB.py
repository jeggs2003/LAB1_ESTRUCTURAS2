...
class ArbolB:
    ...
    def find(self, nombre, reclutador):
        return self._buscar_en_nodo(self.root, nombre, reclutador)

    def _buscar_en_nodo(self, nodo, nombre, reclutador):
        if nodo is None:
            return -1  # Si el nodo es None, el valor no se encontró

        for i in range(nodo.n):
            if nodo.key[i].get_nombre() == nombre and nodo.key[i].get_reclutador() == reclutador:
...
#Find para DPI
    def finddpi(self, dpi, reclutador):
        return self._buscar_en_nododpi(self.root, dpi, reclutador)

    def _buscar_en_nododpi(self, nodo, dpi, reclutador):
        if nodo is None:
            return -1  # Si el nodo es None, el valor no se encontró

        for i in range(nodo.n):
            if nodo.key[i].get_dpi() == dpi and nodo.key[i].get_reclutador() == reclutador:
                compa = nodo.key[i].get_compa()
                encrip = nodo.key[i].get_enc()
                desencriptados = []
                huf_trees = [HuffmanTree(compa[i]) for i in range(len(compa))]
